...
Bossa.KeyframableFilter {
    ...
    property var cornerProperties: ['bossa:corner1', 'bossa:corner2', 'bossa:corner3', 'bossa:corner4']
    ...
    property var cornerStartValues: ['_bossa:corner1StartValue', '_bossa:corner2StartValue', '_bossa:corner3StartValue', '_bossa:corner4StartValue']
    property var cornerMiddleValues: ['_bossa:corner1MiddleValue', '_bossa:corner2MiddleValue', '_bossa:corner3MiddleValue', '_bossa:corner4MiddleValue']
    property var cornerEndValues: ['_bossa:corner1EndValue', '_bossa:corner2EndValue', '_bossa:corner3EndValue', '_bossa:corner4EndValue']
    ...
    function applyTracking(motionTrackerRow, frame) {
        ...
        if (data.length > 0) {
            ...
            // Use a bossa property to backup current values
            if (filter.get('bossa:backup.' + params[0]).length === 0) {
                params.forEach(param => {
                    filter.set('bossa:backup.' + param, filter.getDouble(param));
                });
            } else {
                params.forEach(param => {
                    filter.resetProperty(param);
                    filter.set(param, filter.get('bossa:backup.' + param));
                });
            }
        }
        data.forEach(i => {
    ...
    Bossa.MotionTrackerDialog {
        ...
        onReset: if (filter.keyframeCount(corner1xProperty) > 0 && filter.animateIn <= 0 && filter.animateOut <= 0) {
            ...
            params.forEach(param => {
                filter.resetProperty(param);
                filter.set(param, filter.getDouble('bossa:backup.' + param));
                filter.resetProperty('bossa:backup.' + param);
            });
            filter.blockSignals = false;
            filter.changed();
            filter.animateInChanged();
