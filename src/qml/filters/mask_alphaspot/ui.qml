...
Item {
    ...
    property string rectProperty: 'bossa:rect'
    ...
    property string startValueRect: '_bossa:startValue'
    property string middleValueRect: '_bossa:middleValue'
    property string endValueRect: '_bossa:endValue'
    ...
    function applyTracking(motionTrackerRow, operation, frame) {
        ...
        if (data.length > 0) {
            ...
            if (filter.get('bossa:backup.' + paramHorizontal).length === 0) {
                params.forEach(param => {
                    filter.set('bossa:backup.' + param, filter.getDouble(param));
                });
                filter.set('bossa:backup.rect', filter.getRect(rectProperty));
            } else {
                params.forEach(param => {
                    filter.resetProperty(param);
                    filter.set(param, filter.getDouble('bossa:backup.' + param));
                });
                filter.set(rectProperty, filter.getRect('bossa:backup.rect'));
            }
        }
    ...
    Bossa.MotionTrackerDialog {
        ...
        onReset: if (filter.keyframeCount(rectProperty) > 0 && filter.animateIn <= 0 && filter.animateOut <= 0) {
            ...
            filter.resetProperty(rectProperty);
            filter.set(rectProperty, filter.getRect('bossa:backup.rect'));
            filter.resetProperty('bossa:backup.rect');
            motionTrackerRow = 0;
