...
Rectangle {
    ...
    property color bossaBlue: Qt.rgba(23 / 255, 92 / 255, 118 / 255, 1)
    ...
    function redrawWaveforms() {
        Logic.scrollIfNeeded(settings.timelineScrolling === Bossa.Settings.CenterPlayhead);
    ...
    Connections {
        function onPositionChanged() {
            if (!stopScrolling && settings.timelineScrolling !== Bossa.Settings.NoScrolling) {
                var smooth = settings.timelineScrolling === Bossa.Settings.SmoothScrolling || tracksAreaMouse.containsPress || tracksAreaMouse.skim;
