...
static const char *kBossaDefaultTransition = "lumaMix";
        ...
        if (track) {
            ...
            if (info)
                ...
                case FadeInRole: {
                    ...
                    if (filter && filter->is_valid() && filter->get(kBossaAnimInProperty))
                        return filter->get_int(kBossaAnimInProperty);
                    else
                ...
                case FadeOutRole: {
                    ...
                    if (filter && filter->is_valid() && filter->get(kBossaAnimOutProperty))
                        return filter->get_int(kBossaAnimOutProperty);
                    else
                        return (filter && filter->is_valid()) ? filter->get_length() : 0;
                }
                ...
                case GroupRole:
                    if (info->cut->property_exists(kBossaGroupProperty))
                        return info->cut->get_int(kBossaGroupProperty);
                    else
                        return -1;
...
void MultitrackModel::removeClip(int trackIndex, int clipIndex, bool rippleAllTracks)
{
    ...
    if (track) {
        Mlt::Playlist playlist(*track);
        if (clipIndex < playlist.count()) {
            // Bossa does not like the behavior of remove() on a
...
void MultitrackModel::liftClip(int trackIndex, int clipIndex)
{
    ...
    if (track) {
        Mlt::Playlist playlist(*track);
        if (clipIndex < playlist.count()) {
            // Bossa does not like the behavior of replace_with_blank() on a
            // transition (MLT mix clip). So, we null mlt_mix to prevent it.
            clearMixReferences(trackIndex, clipIndex);
        ...
        if (info && info->producer && info->producer->is_valid()) {
            ...
            if (!info->producer->get("audio_index")
                ...
                if (!filter) {
                    ...
                    f.set(kBossaFilterProperty, "audioGain");
        ...
        if (info && info->producer && info->producer->is_valid()) {
                ...
                if (duration > 0) {
                    ...
                    if (!filter) {
                        if (Settings.playerGPU()) {
                            ...
                            f.set(kBossaFilterProperty, "fadeInMovit");
                            f.set("alpha", i == bottomVideoTrackMltIndex() ? 1 : -1);
                            info->producer->attach(f);
                            filter.reset(new Mlt::Filter(f));
                        } else {
                            Mlt::Filter f(MLT.profile(), "brightness");
                            f.set(kBossaFilterProperty, "fadeInBrightness");
                    ...
                    filter->set(kBossaAnimInProperty, duration);
                    isChanged = true;
                } else if (filter) {
            ...
            if (!info->producer->get("audio_index")
                ...
                if (duration > 0) {
                    ...
                    if (!filter) {
                        Mlt::Filter f(MLT.profile(), "volume");
                        f.set(kBossaFilterProperty, "fadeInVolume");
                        info->producer->attach(f);
                        filter.reset(new Mlt::Filter(f));
                        filter->set_in_and_out(info->frame_in, info->frame_out);
                    } else {
        ...
        if (info && info->producer && info->producer->is_valid()) {
            ...
            if (m_trackList[trackIndex].type == VideoTrackType
                ...
                if (duration > 0) {
                    ...
                    if (!filter) {
                        if (Settings.playerGPU()) {
                            Mlt::Filter f(MLT.profile(), "movit.opacity");
                            f.set(kBossaFilterProperty, "fadeOutMovit");
                            f.set("alpha", i == bottomVideoTrackMltIndex() ? 1 : -1);
                            info->producer->attach(f);
                            filter.reset(new Mlt::Filter(f));
                        } else {
                            Mlt::Filter f(MLT.profile(), "brightness");
                            f.set(kBossaFilterProperty, "fadeOutBrightness");
                            if (i == bottomVideoTrackMltIndex()) {
                                f.set("alpha", 1);
                            } else {
                                f.set("alpha", 0);
                    ...
                    filter->set(kBossaAnimOutProperty, duration);
                    isChanged = true;
                } else if (filter) {
                    // Remove the video filter.
                    info->producer->detach(*filter);
            ...
            if (!info->producer->get("audio_index")
                ...
                if (duration > 0) {
                    ...
                    if (!filter) {
                        Mlt::Filter f(MLT.profile(), "volume");
                        f.set(kBossaFilterProperty, "fadeOutVolume");
                        info->producer->attach(f);
                        filter.reset(new Mlt::Filter(f));
                        filter->set_in_and_out(info->frame_in, info->frame_out);
                    } else {
    ...
    if (track) {
        ...
        if ((targetIndex < clipIndex && endOfCurrentClip > endOfPreviousClip) || // dragged left
            ...
            if (producer.is_valid()) {
                producer.parent().set(kBossaTransitionProperty, kBossaDefaultTransition);
            }
            endInsertRows();
    ...
    if (filter && filter->is_valid()) {
        ...
        if (service.is_valid() && service.get(kMultitrackItemProperty)) {
            ...
            if (parts.length() == 2) {
                QModelIndex modelIndex = createIndex(parts[0].toInt(), 0, parts[1].toInt());
                QVector<int> roles;
                const char *name = filter->get(kBossaFilterProperty);
...
bool MultitrackModel::createIfNeeded()
{
    if (!m_tractor) {
        ...
        m_tractor->set(kBossaXmlProperty, 1);
        retainPlaylist();
        addBackgroundTrack();
        addVideoTrack();
        emit created();
...
bool MultitrackModel::warnIfInvalid(Mlt::Service &service)
{
    if (!service.is_valid()) {
        ...
        QMessageBox::critical(&MAIN,
                              ...
                              tr("Error: Bossa could not find the %1 plugin on your system.\n\n"
...
int MultitrackModel::addAudioTrack()
{
    ...
    // Add the bossa logical audio track.
    Track t;
    t.mlt_index = i;
...
int MultitrackModel::addVideoTrack()
{
    ...
    // Add the bossa logical video track.
    Track t;
    t.mlt_index = i;
...
bool MultitrackModel::isTransition(Mlt::Playlist &playlist, int clipIndex) const
{
    ...
    if (producer && producer->is_valid() && producer->parent().is_valid()
        && producer->parent().get(kBossaTransitionProperty))
        return true;
    return false;
}
    ...
    if (track->is_valid()) {
        ...
        if (transitionIn && isTransition(playlist, clipIndex - 1)) {
            ...
            if (producer.is_valid()) {
                ...
                if (!qstrcmp(track->parent().get(kBossaHashProperty),
                             oldClip.parent().get(kBossaHashProperty))) {
...
void MultitrackModel::refreshTrackList()
{
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (trackId == "black_track")
            ...
            continue;
        else if (!track->get(kBossaPlaylistProperty) && !track->get(kAudioTrackProperty)) {
            int hide = track->get_int("hide");
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (trackId == "black_track")
            ...
            continue;
        else if (!track->get(kBossaPlaylistProperty) && !track->get(kVideoTrackProperty)) {
            int hide = track->get_int("hide");
