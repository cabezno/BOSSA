...
public:
        ...
        : QSortFilterProxyModel(parent)
    {
        ...
        m_functors.push_back([this](int row, const QModelIndex &index) {
            auto clip = MAIN.playlist()->get_clip(row);
            if (clip && clip->is_valid()) {
                return QString::fromUtf8(clip->parent().get(kBossaBinsProperty)).isEmpty();
            }
            return false;
        });
...
Mlt::Playlist *PlaylistDock::binPlaylist()
{
    ...
    for (int i = 0; i < count; ++i) {
        ...
        if (info.producer && info.producer->is_valid()
            && (isAll || bin == info.producer->get(kBossaBinsProperty)))
        ...
        if (p.is_valid()) {
            ...
            if (ui->treeWidget->topLevelItemCount() > SmartBinCount) {
                ...
                if (!items.isEmpty() && items.first()->data(0, Qt::UserRole).isNull()) {
                    auto bin = items.first()->text(0);
                    producer->set(kBossaBinsProperty, bin.toUtf8().constData());
                }
            }
            if (!MLT.isLiveProducer(producer) || producer->get_int(kBossaVirtualClip)) {
...
void PlaylistDock::assignToBin(Mlt::Properties &properties, QString bin)
{
    ...
    properties.set(kBossaBinsProperty, bin.toUtf8().constData());
}
...
void PlaylistDock::onOpenActionTriggered()
{
    ...
    if (i) {
        ...
        bool isNonSequenceImage = MLT.isImageProducer(p) && !p->get_int(kBossaSequenceProperty);
    ...
    if (i) {
        if (qApp->keyboardModifiers() == Qt::ShiftModifier) {
            ...
            bool isNonSequenceImage = MLT.isImageProducer(p)
                                      && !p->get_int(kBossaSequenceProperty);
            emit clipOpened(p, isNonSequenceImage ? false : Settings.playlistAutoplay());
        }
        delete i;
        m_iconsView->resetMultiSelect();
