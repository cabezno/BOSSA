    ...
    : QSpinBox(parent)
{
    ...
    connect(&Settings, &BossaSettings::timeFormatChanged, this, [&]() { setValue(value()); });
}
...
void TimeSpinBox::keyPressEvent(QKeyEvent *event)
{
    if (event->key() == Qt::Key_PageUp || event->key() == Qt::Key_PageDown) {
        // Disable page up & page down (step by 10) since those keys are used for other things in Bossa.
        event->ignore();
        return;
    }
