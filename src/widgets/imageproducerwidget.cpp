...
#define kBossaResourceProperty "bossa_resource"
...
void ImageProducerWidget::setProducer(Mlt::Producer *p)
{
    ...
    QString resource;
    if (m_producer->get(kBossaResourceProperty)) {
        resource = QString::fromUtf8(m_producer->get(kBossaResourceProperty));
    ...
    QString caption = m_producer->get(kBossaCaptionProperty);
    if (caption.isEmpty()) {
        caption = name;
    ...
    ui->sequenceCheckBox->setChecked(m_producer->get_int(kBossaSequenceProperty));
    ui->repeatSpinBox->setEnabled(m_producer->get_int(kBossaSequenceProperty));
    ui->defaultDurationButton->setEnabled(!m_producer->get_int(kBossaSequenceProperty));
...
void ImageProducerWidget::reopen(Mlt::Producer *p)
{
    ...
    if (p->get_int(kBossaSequenceProperty)) {
        MLT.play();
    } else {
        MLT.seek(position);
    }
...
void ImageProducerWidget::recreateProducer()
{
    ...
    p->pass_list(*m_producer,
                 ...
                 ", begin, ttl," kBossaResourceProperty
                 ", autolength, length," kBossaSequenceProperty ", " kPlaylistIndexProperty
...
void ImageProducerWidget::on_sequenceCheckBox_clicked(bool checked)
{
    ...
    if (checked && !m_producer->get(kBossaResourceProperty))
        m_producer->set(kBossaResourceProperty, resource.toUtf8().constData());
    m_producer->set(kBossaSequenceProperty, checked);
    ...
    if (checked) {
        ...
        m_producer->set("resource", m_producer->get(kBossaResourceProperty));
        m_producer->set("length",
...
static QString GetFilenameFromProducer(Mlt::Producer *producer, bool useOriginal)
{
    ...
    } else if (producer->get(kBossaResourceProperty)) {
        resource = QString::fromUtf8(producer->get(kBossaResourceProperty));
    } else {
...
void ImageProducerWidget::on_actionReset_triggered()
{
    ...
    const char *s = m_producer->get(kBossaResourceProperty);
    if (!s)
