...
Mlt::Producer *GlaxnimateProducerWidget::newProducer(Mlt::Profile &profile)
{
    ...
    p->set(kBossaCaptionProperty, m_title.toUtf8().constData());
    p->set(kBossaDetailProperty, filename.toUtf8().constData());
...
void GlaxnimateProducerWidget::setProducer(Mlt::Producer *p)
{
    ...
    if (QString::fromUtf8(p->get(kBossaCaptionProperty)).isEmpty()) {
        p->set(kBossaCaptionProperty, m_title.toUtf8().constData());
    }
    ui->lineEdit->setText(QString::fromUtf8(p->get(kBossaCaptionProperty)));
...
void GlaxnimateProducerWidget::onFileChanged(const QString &path)
{
    ...
    if (m_producer && m_producer->is_valid()) {
        ...
        if (QString::fromUtf8(m_producer->get(kBossaCaptionProperty)) == m_title) {
            m_producer->set(kBossaCaptionProperty, caption.toUtf8().constData());
        }
        m_title = caption;
                ...
                if (info && info->producer && info->producer->is_valid()) {
                    ...
                    for (int i = 0; i < count; i++) {
                        ...
                        if (filter && filter->is_valid()) {
                            if (found
                                || !qstrcmp(filter->get(kBossaFilterProperty), "maskGlaxnimate")) {
                                found = true;
...
void GlaxnimateIpcServer::onReadyRead()
{
    ...
    if (m_isProtocolValid && parent) {
        ...
        if (frameNum != parent->m_frameNum) {
            LOG_DEBUG() << "glaxnimate time =" << time << "=> Bossa frameNum =" << frameNum;
